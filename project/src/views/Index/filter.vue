<template>
	<div id="filter">
		<div class="title">
			<div class="title-item">科目选择</div>
		</div>
		<div class="options">
			<el-cascader v-model="subject" :options="subjectOptions" :props="{ expandTrigger: 'hover' }"
				@change="subjectChange"></el-cascader>
			<!-- <span class="more">
				更多<b>>></b>
			</span> -->
		</div>
...
export default {
	...
	methods: {
		...
		async getChildrenOptions() {
			let res = await this.axios.get(`${this.baseUrl}/class/subjects/get`)
			if (res.data.status == "success") {
				this.subjectOptions = res.data.data
				this.subject = [
					this.subjectOptions[0].value,
					this.subjectOptions[0].children[0].value,
				]
				this.getClass()
			}
		},
		...
		async getClass() {
			let res = await this.axios.get(`${this.baseUrl}/class/subject/filter`, {
				params: {
