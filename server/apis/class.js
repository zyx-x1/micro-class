...
module.exports = {
  ...
  async searchClass(req, res, next) {
    ...
    let finalResult = [];
    let end =
      (curr_page + 1) * page_size > total ? total : (curr_page + 1) * page_size;
    finalResult =
      total > page_size ? result.slice(curr_page * page_size, end) : result;
    return res.json({
      status: "success",
      data: finalResult,
      total,
  ...
  async getSubjects(req, res, next) {
    ...
    elementarys = [
      ...new Set(
        elementarys.map((el) =>
          el.knowledge_information.split("|")[0].replace("小学", "")
        )
      ),
    ];
    ...
    middles = [
      ...new Set(
        middles.map((el) =>
          el.knowledge_information.split("|")[0].replace("初中", "")
        )
      ),
    ];
    ...
    highs = [
      ...new Set(
        highs.map((el) =>
          el.knowledge_information.split("|")[0].replace("高中", "")
        )
      ),
    ];
  ...
  async downloadFile(req, res, next) {
    ...
    await db(
      ...
      [
        ...
        "deduct",
        "下载附件消耗",
        parseInt(fileCredits),
        new Date(),
      ]
    );
    await db(
      `insert into micro_class_file_download_record (user_id,class_id,file_id,credits,user_surplus_credits,date) values (?,?,?,?,?,?)`,
      [
        parseInt(userId),
        parseInt(classId),
        parseInt(fileId),
        parseInt(fileCredits),
        userCredits - fileCredits,
        new Date(),
      ]
    );
    return res.json({
