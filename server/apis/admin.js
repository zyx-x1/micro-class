...
module.exports = {
  ...
  async adminLogin(req, res, next) {
    let user = (
      await db(
        `select password from user where user_type='admin' and email="${req.query.email}"`
      )
    )[0];
    if (!user) {
  ...
  async submitAudit(req, res, next) {
    ...
    if (result.protocol41) {
      let creator = await db("select email from user where id=?", [creatorId]);
      console.log(creator);
      if (!creator[0]) {
        return res.json({
          status: "success",
      ...
      console.log(creator);
      let userEmail = creator[0].email;
      let messageSql = `insert into message (receiver_email,class_id,date,type,content) values (?,?,?,?,?)`;
      let messageData = [
        userEmail,
        id,
        new Date(),
  ...
  async getDownloadRecord(req, res, next) {
    let { currPage, pageSize } = req.query;
    let totalResult = await db(
      `select count(id) from micro_class_file_download_record`
    );
    let result = await db(
      `select * from micro_class_file_download_record order by date desc limit ?,?`,
      [pageSize * (currPage - 1), parseInt(pageSize)]
    );
    result.forEach(async (item, index) => {
      let fileId = item.file_id;
      let file = await db(`select * from micro_class_file where id=?`, [
        fileId,
      ]);
      item._file = file[0];
      let userId = item.user_id;
      let user = await db(`select * from user where id=?`, [userId]);
      item._user = user[0];
